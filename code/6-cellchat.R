...
test <- pbmc
table(test$group)
test$celltype[test$celltype=="Endothelial_cell"] <- paste("Endothelial_cell",endo$seurat_clusters[match(colnames(endo),colnames(test)[test$celltype=="Endothelial_cell"])],sep="_")
Idents(object= test) <- test@meta.data$celltype
test <- subset(test, downsample=10000)
DimPlot(test)
# data.input <- GetAssayData(test, assay = "RNA", slot = "data")
# labels <- Idents(test)
...
# ##提取表达矩阵和细胞分类信息
# 
# DimPlot(test, label = T, group.by = "celltype", reduction = "umap")
# DimPlot(test, label = T, group.by = "tumor.ident", reduction = "umap")
data.input <- GetAssayData(test, assay = "RNA", slot = "data")
identity <- subset(test@meta.data, select = "celltype")
...
pathways.show <- "TGFb"            #指定需要展示的通路

# ?netVisual_aggregate
##层次图
vertex.receiver = c(1,5,8,9) # a numeric vector
vertex.receiver = c(2,3,4,6,7,10,11)
netVisual_aggregate(cellchat, signaling = "TGFb",  vertex.receiver = vertex.receiver,layout="hierarchy")
...
pairLR.SPP1 <- extractEnrichedLR(cellchat, signaling = "TGFb", geneLR.return = FALSE)
...
##圈图
netVisual_individual(cellchat, signaling = "TGFb", pairLR.use = LR.show, layout = "circle")
##和弦图
...
##气泡图
levels(cellchat@idents)
pdf("plot_new/TGFb_endo.pdf")
netVisual_bubble(cellchat, sources.use = 2, targets.use = c(1,11:20), remove.isolate = FALSE,signaling =  c("TGFb"))
netVisual_bubble(cellchat, sources.use = c(1,11:20), targets.use = 2, remove.isolate = FALSE,signaling =  c("TGFb"))
netVisual_bubble(cellchat, sources.use = 5, targets.use = c(1,11:20), remove.isolate = FALSE,signaling =  c("TGFb"))
netVisual_bubble(cellchat, sources.use = c(1,11:20), targets.use = 5, remove.isolate = FALSE,signaling =  c("TGFb"))
dev.off()
##sources.use = 2 是值第二个细胞亚群
netVisual_bubble(cellchat, sources.use =vertex.receiver, targets.use = c(1:5), remove.isolate = FALSE)
##指定信号通路
cellchat@netP$pathways 
netVisual_bubble(cellchat, sources.use =vertex.receiver, targets.use =vertex.receiver,signaling =  c("SPP1","CCL"), remove.isolate = FALSE)


pairLR  <- extractEnrichedLR(cellchat, signaling =c("TGFb"), geneLR.return = FALSE)
netVisual_bubble(cellchat, targets.use =vertex.receiver, sources.use = setdiff(1:length(levels(cellchat@idents)),vertex.receiver),
                 pairLR.use =pairLR , remove.isolate = FALSE)
netVisual_bubble(cellchat, sources.use =vertex.receiver, targets.use = setdiff(1:length(levels(cellchat@idents)),vertex.receiver),
                 pairLR.use =pairLR , remove.isolate = FALSE)
#和弦图               
netVisual_chord_gene(cellchat, sources.use = 2, targets.use = c(1:10), lab.cex = 0.5,legend.pos.y = 30)
...
netAnalysis_signalingRole_network(cellchat, signaling = "TGFb", width = 8, height = 2.5, font.size = 10)
...
#############################################################################################
#不同分组之间的配对分析
sc.sp=SplitObject(test,split.by = "group")
sc.11=test[,sample(colnames(sc.sp[["CEAS"]]),5000)]
sc.3=test[,sample(colnames(sc.sp[["HC"]]),5000)]
...
groupSize <- as.numeric(table(cc.sc11@idents))
par(mfrow = c(1,2), xpd=TRUE)
netVisual_circle(cc.sc11@net$count, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Number of interactions")
netVisual_circle(cc.sc11@net$weight, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Interaction weights/strength")

plotGeneExpression(cc.sc11, signaling = "SELE")
...
cellchat <- netAnalysis_computeCentrality(cellchat, slot.name = "netP")
cc.sc3 = cellchat
cc.sc3@netP$pathways

groupSize <- as.numeric(table(cc.sc3@idents))
par(mfrow = c(1,2), xpd=TRUE)
netVisual_circle(cc.sc3@net$count, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Number of interactions")
netVisual_circle(cc.sc3@net$weight, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Interaction weights/strength")

plotGeneExpression(cc.sc3, signaling = "SELE")
#默认情况下，plotGeneExpression只显示与推断的重要通信相关的信号基因的表达。
#用户可以通过显示一个信号通路相关的所有信号基因的表达。
plotGeneExpression(cc.sc3, signaling = "SELE", enriched.only = FALSE)
#也可以用气泡图展示
plotGeneExpression(cc.sc3, signaling = "SELE",type = "dot",color.use = col.ls)

##############################################
cc.list=list(HC=cc.sc3,CEAS=cc.sc11)
cellchat=mergeCellChat(cc.list,cell.prefix = T,add.names = names(cc.list))
##可视化
##所有细胞群总体观：通讯数量与强度对比
pdf("plot_new/cellchat.pdf", height = 8, width = 8)
...
##红色是case相对于HC上调的，蓝色是下调的

#数量与强度差异热图
netVisual_heatmap(cellchat)
netVisual_heatmap(cellchat,measure = "weight")
#case和HC对比，红色是上调，蓝色是下调
...
netVisual_circle(cc.list[[1]]@net$count,weight.scale = T,label.edge = F,
                 edge.weight.max =weight.max[2],edge.width.max = 12,title.name = "HC" )

netVisual_circle(cc.list[[2]]@net$count,weight.scale = T,label.edge = F,
                 edge.weight.max =weight.max[2],edge.width.max = 12,title.name = "CEAS" )
dev.off()

table(test@active.ident)
s.cell=c( "Endothelial_cell_0", "Endothelial_cell_3")
...
netVisual_heatmap(cellchat, signaling = "SELE", color.heatmap = "Reds")
